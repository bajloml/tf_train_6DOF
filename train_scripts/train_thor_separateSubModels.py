...
if __name__ == '__main__':
    ...
    parser.add_argument("--pretrained", default=True, help='do you want to use vgg imagenet pretrained weights')
    ...
    # convert to boolean
    if (opt.pretrained == 'False') or (opt.pretrained ==  'false'):
        opt.pretrained = False
    else:
        opt.pretrained = True
    
    # convert to boolean
    if (opt.savemodelafterepoch == 'False') or (opt.savemodelafterepoch == 'false'):
        opt.savemodelafterepoch = False
    else:
        opt.savemodelafterepoch = True

    #########################################Create debug folder to store the debug data##################################################
    dir_path = os.path.dirname(os.path.realpath(__file__))
    #debugFolderPath = "{}{}{}".format(dir_path, '\\DEBUG_', str(opt.outf))
    ...
    # dataset images path
    # trainDatasetImagePath = '{}\\dataset_imgs\\train'.format(debugFolderPath)
    ...
    #testDatasetImagePath = '{}\\dataset_imgs\\test'.format(debugFolderPath)
    ...
    #labelsLogitsImgPath = '{}\\logits_labels_img_epoch_end'.format(debugFolderPath)
    ...
    #modelFolder = '{}\\models\\'.format(debugFolderPath)
    #ckptFolder = '{}\\ckpt\\'.format(debugFolderPath)
...
####################################################################################################
    ...
    with tf.device('/GPU:0'):
        try:
            ...
            tf.keras.utils.plot_model(model=netModel.modelForPlot(),to_file = os.path.join(debugFolderPath, 
                                                                                           '{}_{}.png'.format(opt.namefile, 'Plot_Model')),
                                                                                           show_shapes=True,
            ...
            # train_log_dir = os.path.join(debugFolderPath, '{}'.format(debugFolderPath, 'logs', 'gradient_tape', current_time, 'train'))
            ...
            for epoch in epochRange:
                ...
                for step, batch_train in enumerate(train_dataset):
                    ...
                    if debug:
                        ...
                        imgBelLog, imgBelLab, imgAffLog, imgAffLab = getLabelsLogitsImages(logits_beliefs, logits_affinities, batch_train, 0)
                        
                        # save images
                        imgBelLog.convert('RGB').save(os.path.join(labelsLogitsImgPath, '{}_{}.png'.format(str(epoch), 'Belief_Logit')))
                        imgBelLab.convert('RGB').save(os.path.join(labelsLogitsImgPath, '{}_{}.png'.format(str(epoch), 'Belief_Label')))
                        imgAffLog.convert('RGB').save(os.path.join(labelsLogitsImgPath, '{}_{}.png'.format(str(epoch), 'Affinity_Logit')))
                        imgAffLab.convert('RGB').save(os.path.join(labelsLogitsImgPath, '{}_{}.png'.format(str(epoch), 'Affinity_Label')))
            ...
            plt.savefig(os.path.join(debugFolderPath, 'affinities&beliefs_loss.png'), bbox_inches='tight')
